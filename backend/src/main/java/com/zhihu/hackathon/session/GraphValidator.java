...
public class GraphValidator {
  public ValidGraph validate(String target, Graph input) {
    if (input == null || input.nodes() == null || input.edges() == null
        || input.nodes().size() > 20 || input.edges().size() > 210) invalid("GRAPH_SIZE_LIMIT");
    String targetDescription = normalize(input.targetDescription());
    if (targetDescription.length() > 1000) invalid("DESCRIPTION_TOO_LONG");
    Map<String, Node> nodes = new LinkedHashMap<>();
    Set<String> names = new HashSet<>();
    names.add(normalize(target).toLowerCase(Locale.ROOT));
    for (Node n : input.nodes()) {
      if (n == null || n.key() == null || !n.key().matches("[a-zA-Z0-9_-]{1,40}") || n.key().equals("target")) invalid("INVALID_NODE_KEY");
      ...
      if (name.isEmpty() || name.length() > 100 || description.length() > 1000
          || !names.add(name.toLowerCase(Locale.ROOT)) || nodes.containsKey(n.key())) invalid("INVALID_OR_DUPLICATE_NODE");
    ...
    for (Edge e : input.edges()) {
      if (e == null || !keys.contains(e.from()) || !keys.contains(e.to()) || e.from().equals(e.to())
          || e.from().equals("target") || !next.get(e.from()).add(e.to())) invalid("INVALID_OR_DUPLICATE_EDGE");
      degree.merge(e.to(), 1, Integer::sum);
    }
    // 在有向无环图中，唯一汇点为 target 即保证所有前置节点可达目标。
    for (String key : nodes.keySet()) if (next.get(key).isEmpty()) invalid("NODE_CANNOT_REACH_TARGET");
    ...
    if (visited != keys.size()) invalid("GRAPH_CYCLE");
  ...
  public List<Question> validateQuestions(List<SavedNode> nodes, List<Question> questions) {
    if (questions == null || questions.size() != nodes.size()) invalid("QUESTION_COUNT_MISMATCH");
  ...
  private static void invalid(String reason) { throw new IllegalArgumentException(reason); }
}
