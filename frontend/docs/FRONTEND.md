...
知阶（Knowledge Steps）是面向“想学新知识但不知道缺什么”的学生与自学者的轻量知识寻路工具。核心承诺是：用户输入想学什么，系统推导出前置知识，通过少量自评确认用户基础，保留全部节点并按熟悉程度分配资料，最终给出可执行的待补齐路径，并让每个知识节点都能直接进入知乎资料学习。
...
1. **服务端是权威**：已掌握节点仍然展示，不搜索资料；前端不要自行发明“删除已掌握节点”的逻辑。
...
3. VERY_FAMILIAR 为已掌握；其余三项为待补齐。结果展示四档原始答案，资料上限依次为 0/2/3/5。
...
本次四档资料规则、旧任务迁移和搜索恢复行为详见 [自评与资料规则](../../docs/ASSESSMENT_RESOURCES.md)。


### 等待页动画（2026-09-12）

等待卡片采用用户指定的 dylanharriscameron 蓝白光斑样式，图谱阶段展示 24 段方块进度条及预估百分比。当前模型调用非流式，不提供真实百分比：预估缓慢增长至 95%，收到图谱完成状态后，用 0.9 秒加速填充至 100%，停留 0.25 秒再结束等待展示；刷新页面重新估算，不影响实际请求。步骤一到二的纵向进度条与方块和数字共用一个单调递增的预估值，不循环。若轮询直接跳到 READY，仍保留完成动画时间，实际问卷请求不延迟。资料搜索仍使用后端实际节点计数；减少动态效果时停止装饰动画。


### 零节点提醒

问卷为空或结果图仅包含目标时，展示简约提示弹窗“暂未找到前置知识”，建议检查名称、错别字并补充领域或研究角度，不再展示 0/0 问卷和“无需额外确认”。点击“修改目标后重试”返回首页并保留原目标供编辑。该判断依据是否存在前置节点，不能使用 missingCount（全部非常了解时也为零）。后端仍保留空图记录，不将可能的输入歧义断言为用户错误。


### 答题页布局更新

答题页改为简约白底双栏工作区，去除五层圆环、玻璃叠层和倾斜效果。桌面左侧内容决定整体高度，右侧判断栏保持等高，标题固定、列表内部滚动，支持键盘聚焦滚动区。760px 以下改为上下布局，判断列表最多 280px 高。原有作答、复核、跳题、保存禁用和失败提示行为保留。实现集中在 design/quiz.css。
