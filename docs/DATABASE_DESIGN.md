...
> 第一版字段设计，已提供 V1/V2 迁移；具体数据库是否已迁移须检查其 Flyway 历史，不代表业务接口已实现。不要直接删除现有表。接口见 [接口约定](API_CONTRACT.md)，迁移操作见 [迁移说明](FLYWAY.md)。
...
当前实现进展：创建任务、图与依赖保存、节点搜索结果、自评题保存及进度查询已使用上述表；本地测试身份写入带 local-test: 前缀的用户。答案与最终结果业务仍待实现。本轮未修改 V1/V2 或增加表。数据库连接设置 5 秒忙等待，生成链路使用短事务；启动时把生成中任务标记为失败以避免永久停留。
