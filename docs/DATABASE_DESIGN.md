...
当前实现进展：创建任务、图与依赖保存、节点搜索结果、自评题保存、答案保存、完成结果推导、节点资料读取及进度查询已使用上述表；本地测试身份写入带 local-test: 前缀的用户。V3 更新自评资料策略，不修改已应用的 V1/V2。数据库连接设置 5 秒忙等待，生成链路使用短事务；启动时将中断的图谱/题目生成标记失败，将已完整作答的搜索中任务恢复 READY 以便重新提交继续搜索。
...
新建 learning_sessions.id 使用 41 位毫秒时间、10 位 worker ID、12 位序列组成的正整数，纪元为 2024-01-01 UTC。API 将 ID 序列化为字符串；旧数字 ID 与所有外键保持兼容。

V4 新增 session_snowflake_state(worker_id, last_tick)，原子更新持久化逻辑时间和序列。时钟回拨时沿用逻辑时间，同毫秒超过 4096 个 ID 时推进逻辑毫秒；事务回滚时同时回滚分配状态。节点、题目等 ID 规则不变。

V5：node_resources.content_date 为可空 TEXT（YYYY-MM-DD），保存知乎发布时间或更新时间的日级日期，与 fetched_at 抓取时间分离。

寻路硬删除：应用按外键依赖顺序，在单事务内删除答案、题目、资料、连线、节点、寻路。不存在软删除标记；账号及雪花分配状态不属于单条寻路，不删除。后台资源写入同时检查 session_id 与 node_id，避免 SQLite 节点 ID 复用造成跨寻路污染。
