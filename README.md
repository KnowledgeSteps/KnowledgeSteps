...
当前已实现创建寻路任务、查询状态、答卷读取、答案保存与结果生成，以及“模型生成依赖图 → 校验分层 → 知乎搜索 → 生成自评题”的异步链路。节点资料和 OAuth 登录仍待实现。本地开发用 local-test 测试身份；普通配置未登录返回 401。产品范围见 [项目计划](docs/PROJECT.md)，接口及 Apifox 调试步骤见 [接口约定](docs/API_CONTRACT.md)。
